...
class derechosValidatorUser extends sfValidatorBase
{
     ...
     protected function doClean($values)
    {
        ...
        if ($username && $password)
        {
                ...
                //$user = UsuariosPeer::retrieveByPK(strtoupper($username));
//                $modelo = sfConfig::get('app_derechos_plugin_modelo').'Peer';
//                var_dump($modelo);die();
//                $user = $modelo::login($username, $password);
            switch (sfConfig::get('app_derechos_plugin_modelo')){
                case 'Administrador':
                    $user = AdministradorPeer::login($username, $password);
                    break;
                case 'Chicas':
                    $user = ChicasPeer::login($username, $password);
                    break;
                case 'Clientes':
                    $user = ClientesPeer::login($username, $password);
                    break;
                case 'Reevendedores':
                    $user = ReevendedoresPeer::login($username, $password);
                    break;
            }
                // existe?
                if($user)
